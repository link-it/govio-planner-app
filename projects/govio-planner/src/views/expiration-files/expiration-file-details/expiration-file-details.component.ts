...
import { ExpirationFile } from './expiration-file';

declare const saveAs: any;

@Component({
  selector: 'app-expiration-file-details',
  templateUrl: 'expiration-file-details.component.html',
  styleUrls: ['expiration-file-details.component.scss']
})
export class ExpirationFileDetailsComponent implements OnInit, OnChanges, AfterContentChecked, OnDestroy {
  static readonly Name = 'ExpirationFileDetailsComponent';
  ...
  _file: ExpirationFile = new ExpirationFile({});
  ...
  _loadFile() {
    if (this.id) {
      ...
      this.apiService.getDetails(this.model, this.id).subscribe({
        next: (response: any) => {
          this.file = this._prepareFileData(response); // new File({ ...response });
          this._file = this.file; // new File({ ...response });
  ...
  _initBreadcrumb() {
    ...
    this.breadcrumbs = [
      ...
      { label: 'APP.TITLE.ExpirationFiles', url: `/${this.model}`, type: 'link' },
